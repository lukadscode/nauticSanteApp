...
const VideoExerciseScreen = ({route}) => {
  ...
  const embedUrl = getVimeoEmbedUrl(exercise?.videos[0].link);

  if (!exercise || !embedUrl) {
    return (
      <SafeAreaView style={styles.container}>
        <Text style={styles.errorText}>Aucune vidéo trouvée ou lien invalide
          ({exercise?.exercise?.videos[0].link}).</Text>
      </SafeAreaView>
    );
  }
